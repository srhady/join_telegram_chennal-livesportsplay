...
(async () => {
    ...
    try {
        ...
        // একদম বুলেটপ্রুফ লজিক
        const matchesData = await p.evaluate(() => {
            let linkMap = {};
            // শুধু টেবিলের সারি (tr) এর ভেতরের লিংকগুলো ধরবে, এতে মেন্যু বাটনগুলো আসবে না
            let links = Array.from(document.querySelectorAll('tr a'));
            
            for (let a of links) {
                let text = (a.textContent || '').replace(/\s+/g, ' ').trim();
                let href = a.href || '';
                
                // crichd এর লিংক হতে হবে এবং হাবিজাবি (score, watch) হওয়া যাবে না
                if (href.includes('crichd.com.co') && text.length > 3 && !/score|watch|live now|telegram|contact|about/i.test(text) && !href.includes('javascript')) {
