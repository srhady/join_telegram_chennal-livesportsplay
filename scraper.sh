...
while true; do
  ...
  # আইডিগুলো বের করে আনা
  IDS=$(curl -s -L -X GET "https://api.bongo-solutions.com/ironman/api/v2/widget/watch-free?limit=$LIMIT&offset=$OFFSET" \
    -H "authorization: Bearer $AUTH_TOKEN" \
    -H "country-code: QkQ%3D" \
    -H "platform-id: abfea462-f64d-491e-9cd9-75ee001f45b0" | jq -r '.items[]? | .firstEpisode // .id')

  # যদি আর কোনো আইডি না থাকে, লুপ বন্ধ হবে
  if [ -z "$IDS" ]; then
    echo "No more videos found. Ending loop."
    break
  fi

  # প্রত্যেকটা আইডির জন্য বিস্তারিত ডেটা আনা
  for id in $IDS; do
    echo "Extracting details for video ID: $id"
    ...
    # Raw ডেটাটাই সুন্দর করে ফাইলে ঢুকানো হচ্ছে
    echo "$DETAIL" >> $OUTPUT_FILE
    
    # সার্ভারকে সন্দেহমুক্ত রাখতে ১ সেকেন্ড বিরতি
    sleep 1
  done

  # পরবর্তী ১০০টা ভিডিওর জন্য অফসেট বাড়ানো
  OFFSET=$((OFFSET + LIMIT))
done

echo "]" >> $OUTPUT_FILE
echo "Success! All detailed data perfectly saved in $OUTPUT_FILE."
